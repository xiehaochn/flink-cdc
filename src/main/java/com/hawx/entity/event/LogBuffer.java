...
public class LogBuffer {
  ...
  public final int getUint24(final int pos) {
    ...
    byte[] buf = buffer;
    return (0xff & buf[position])
        | ((0xff & buf[position + 1]) << 8)
  ...
  public final int getBeInt32(final int pos) {
    ...
    byte[] buf = buffer;
    return (0xff & buf[position + 3])
        | ((0xff & buf[position + 2]) << 8)
  ...
  /**
   * Return packed number from buffer. (little-endian) A Packed Integer has the capacity of storing
   * up to 8-byte integers, while small integers still can use 1, 3, or 4 bytes. The value of the
   * first byte determines how to read the number, according to the following table.
   *
   * <ul>
   *   <li>0-250 The first byte is the number (in the range 0-250). No additional bytes are used.
   *   <li>252 Two more bytes are used. The number is in the range 251-0xffff.
   *   <li>253 Three more bytes are used. The number is in the range 0xffff-0xffffff.
   *   <li>254 Eight more bytes are used. The number is in the range 0xffffff-0xffffffffffffffff.
   * </ul>
   *
   * That representation allows a first byte value of 251 to represent the SQL NULL value.
   */
  ...
  /**
   * Return fix-length string from buffer without null-terminate checking. Fix bug #17 {@link
   * https://github.com/AlibabaTech/canal/issues/17 }
   */
  ...
  /**
   * Return next fix-length string from buffer without null-terminate checking. Fix bug #17 {@link
   * https ://github.com/AlibabaTech/canal/issues/17 }
   */
  ...
  private final BigDecimal getDecimal0(
    ...
    if (mask != 0) /* decimal sign */ buf[pos++] = ('-');
    ...
    for (final int stop = from + intg0 * SIZE_OF_INT32; from < stop; from += SIZE_OF_INT32) {
      ...
      if (x != 0) {
        ...
        for (int i = DIG_PER_DEC1; i > 0; i--) buf[pos++] = ('0');
      }
    }
