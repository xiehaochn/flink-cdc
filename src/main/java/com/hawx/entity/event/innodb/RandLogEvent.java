...
/**
 * Logs random seed used by the next RAND(), and by PASSWORD() in 4.1.0. 4.1.1 does not need it
 * (it's repeatable again) so this event needn't be written in 4.1.1 for PASSWORD() (but the fact
 * that it is written is just a waste, it does not cause bugs). The state of the random number
 * generation consists of 128 bits, which are stored internally as two 64-bit numbers. Binary Format
...
public final class RandLogEvent extends LogEvent {
   ...
   *
   * <ul>
   *   <li>8 bytes. The value for the first seed.
   *   <li>8 bytes. The value for the second seed.
   * </ul>
   *
