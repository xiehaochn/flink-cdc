...
public final class RotateLogEvent extends LogEvent {
   ...
   *
   * <ul>
   *   <li>8 bytes. The position of the first event in the next log file. Always contains the number
   *       4 (meaning the next event starts at position 4 in the next binary log). This field is not
   *       present in v1; presumably the value is assumed to be 4.
   * </ul>
   *
   * <p>Variable data part:
   *
   * <ul>
   *   <li>The name of the next binary log. The filename is not null-terminated. Its length is the
   *       event size minus the size of the fixed parts.
   * </ul>
   *
  ...
  /**
   * Creates a new <code>Rotate_log_event</code> without log information. This is used to generate
   * missing log rotation events.
   */
