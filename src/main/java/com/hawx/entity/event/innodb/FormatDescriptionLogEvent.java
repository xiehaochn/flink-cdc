...
/**
 * For binlog version 4. This event is saved by threads which read it, as they need it for future
...
public final class FormatDescriptionLogEvent extends StartLogEventV3 {
  ...
  /**
   * The number of types we handle in Format_description_log_event (UNKNOWN_EVENT is not to be
   * handled, it does not exist in binlogs, it does not have a format).
   */
  ...
  public static final long checksumVersionProduct =
      (checksumVersionSplit[0] * 256 + checksumVersionSplit[1]) * 256 + checksumVersionSplit[2];
  /**
   * The size of the fixed header which _all_ events have (for binlogs written by this version, this
   * is equal to LOG_EVENT_HEADER_LEN), except FORMAT_DESCRIPTION_EVENT and ROTATE_EVENT (those have
   * a header of size LOG_EVENT_MINIMAL_HEADER_LEN).
   */
