...
/**
 * The Common-Header, documented in the table @ref Table_common_header "below", always has the same
 * form and length within one version of MySQL. Each event type specifies a format and length of the
 * Post-Header. The length of the Common-Header is the same for all events of the same type. The
 * Body may be of different format and length even for different events of the same type. The binary
 * formats of Post-Header and Body are documented separately in each subclass. The binary format of
...
public final class LogHeader {
   ...
   * making SHOW SLAVE STATUS able to print coordinates of the event in the master's binlog). Note:
   * when a transaction is written by the master to its binlog (wrapped in BEGIN/COMMIT) the log_pos
   * of all the queries it contains is the one of the BEGIN (this way, when one does SHOW SLAVE
   * STATUS it sees the offset of the BEGIN, which is logical as rollback may occur), except the
   * COMMIT query which has its real offset.
   */
  protected long logPos;

  /**
   * Timestamp on the master(for debugging and replication of NOW()/TIMESTAMP). It is important for
   * queries and LOAD DATA INFILE. This is set at the event's creation time, except for Query and
   * Load (et al.) events where this is set at the query's execution time, which guarantees good
   * replication (otherwise, we could have a query and its event with different timestamps).
   */
  protected long when;
  ...
  /**
   * The master's server id (is preserved in the relay log; used to prevent from infinite loops in
  ...
  /**
   * The value is set by caller of FD constructor and Log_event::write_header() for the rest. In the
   * FD case it's propagated into the last byte of post_header_len[] at FD::write(). On the slave
   * side the value is assigned from post_header_len[last] of the last seen FD event.
   */
  ...
  public LogHeader(LogBuffer buffer, FormatDescriptionLogEvent descriptionEvent) {
    ...
    if (descriptionEvent.binlogVersion == 3
        && type < LogEvent.FORMAT_DESCRIPTION_EVENT
        && logPos != 0) {
      /*
