 ...
 * Format_description_log_event derives from Start_log_event_v3; it is the Start_log_event of binlog
 * format 4 (MySQL 5.0), that is, the event that describes the other events'
 * Common-Header/Post-Header lengths. This event is sent by MySQL 5.0 whenever it starts sending a
 * new binlog if the requested position is >4 (otherwise if ==4 the event will be sent naturally).
 *
...
public class StartLogEventV3 extends LogEvent {
  ...
  /**
   * We could have used SERVER_VERSION_LENGTH, but this introduces an obscure dependency - if
   * somebody decided to change SERVER_VERSION_LENGTH this would break the replication protocol
   */
