...
/**
 * Event responsible for LOAD DATA execution, it similar to Query_log_event but before executing the
 * query it substitutes original filename in LOAD DATA query with name of temporary file.
 *
 * <ul>
 *   <li>4 bytes. The ID of the file to load.
 *   <li>4 bytes. The start position within the statement for filename substitution.
 *   <li>4 bytes. The end position within the statement for filename substitution.
 *   <li>1 byte. How to handle duplicates: LOAD_DUP_ERROR = 0, LOAD_DUP_IGNORE = 1, LOAD_DUP_REPLACE
 *       = 2
 * </ul>
 *
...
public final class ExecuteLoadQueryLogEvent extends QueryLogEvent {
  ...
  /**
   * We have to store type of duplicate handling explicitly, because for LOAD DATA it also depends
   * on LOCAL option. And this part of query will be rewritten during replication so this
  ...
  public ExecuteLoadQueryLogEvent(
