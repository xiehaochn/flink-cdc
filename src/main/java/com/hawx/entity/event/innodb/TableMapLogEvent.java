...
public final class TableMapLogEvent extends LogEvent {
   ...
   *
   * <ul>
   *   <li>1 byte. The length of the database name.
   *   <li>Variable-sized. The database name (null-terminated).
   *   <li>1 byte. The length of the table name.
   *   <li>Variable-sized. The table name (null-terminated).
   *   <li>Packed integer. The number of columns in the table.
   *   <li>Variable-sized. An array of column types, one byte per column.
   *   <li>Packed integer. The length of the metadata block.
   *   <li>Variable-sized. The metadata block; see log_event.h for contents and format.
   *   <li>Variable-sized. Bit-field indicating whether each column can be NULL, one bit per column.
        ...
        case MYSQL_TYPE_ENUM:
          ...
          logger.warn(
              "This enumeration value is only used internally "
                  + "and cannot exist in a binlog: type="
                  + info.type);
          break;
