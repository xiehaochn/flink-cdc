...
public abstract class RowsLogEvent extends LogEvent {
   ...
   *
   * <ul>
   *   <li>6 bytes. The table ID.
   *   <li>2 bytes. Reserved for future use.
   * </ul>
   *
   * <p>Variable data part:
   *
   * <ul>
   *   <li>Packed integer. The number of columns in the table.
   *   <li>Variable-sized. Bit-field indicating whether each column is used, one bit per column. For
   *       this field, the amount of storage required for N columns is INT((N+7)/8) bytes.
   *   <li>Variable-sized (for UPDATE_ROWS_LOG_EVENT only). Bit-field indicating whether each column
   *       is used in the UPDATE_ROWS_LOG_EVENT after-image; one bit per column. For this field, the
   *       amount of storage required for N columns is INT((N+7)/8) bytes.
   *   <li>Variable-sized. A sequence of zero or more rows. The end is determined by the size of the
   *       event. Each row has the following format:
   *       <ul>
   *         <li>Variable-sized. Bit-field indicating whether each field in the row is NULL. Only
   *             columns that are "used" according to the second field in the variable data part are
   *             listed here. If the second field in the variable data part has N one-bits, the
   *             amount of storage required for this field is INT((N+7)/8) bytes.
   *         <li>Variable-sized. The row-image, containing values of all table fields. This only
   *             lists table fields that are used (according to the second field of the variable
   *             data part) and non-NULL (according to the previous field). In other words, the
   *             number of values listed here is equal to the number of zero bits in the previous
   *             field (not counting padding bits in the last byte). The format of each value is
   *             described in the log_event_print_value() function in log_event.cc.
   *         <li>(for UPDATE_ROWS_EVENT only) the previous two fields are repeated, representing a
   *             second table row.
   *       </ul>
   * </ul>
   *
  ...
  /**
   * Bitmap for columns available in the after image, if present. These fields are only available
   * for Update_rows events. Observe that the width of both the before image COLS vector and the
   * after image COLS vector is the same: the number of columns of the table on the master.
   */
  ...
  /**
   * enum enum_flag These definitions allow you to combine the flags into an appropriate flag set
   * using the normal bitwise operators. The implicit conversion from an enum-constant to an integer
   * is accepted by the compiler, which is then used to set the real set of flags.
   */
  ...
  /**
   * Indicates that rows in this event are complete, that is contain values for all columns of the
   * table.
   */
  ...
  public RowsLogEvent(
    ...
    if (header.type == UPDATE_ROWS_EVENT_V1
        || header.type == UPDATE_ROWS_EVENT
