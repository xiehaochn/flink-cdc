...
/**
 * User_var_log_event. Every time a query uses the value of a user variable, a User_var_log_event is
...
public final class UserVarLogEvent extends LogEvent {
   ...
   *
   * <ul>
   *   <li>4 bytes. the size of the user variable name.
   *   <li>The user variable name.
   *   <li>1 byte. Non-zero if the variable value is the SQL NULL value, 0 otherwise. If this byte
   *       is 0, the following parts exist in the event.
   *   <li>1 byte. The user variable type. The value corresponds to elements of enum Item_result
   *       defined in include/mysql_com.h.
   *   <li>4 bytes. The number of the character set for the user variable (needed for a string
   *       variable). The character set number is really a collation number that indicates a
   *       character set/collation pair.
   *   <li>4 bytes. The size of the user variable value (corresponds to member val_len of class
   *       Item_string).
   *   <li>Variable-sized. For a string variable, this is the string. For a float or integer
   *       variable, this is its value in 8 bytes.
   * </ul>
   *
