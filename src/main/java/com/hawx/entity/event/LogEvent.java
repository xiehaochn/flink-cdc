package com.hawx.entity.event;

import com.hawx.entity.event.innodb.LogHeader;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;

/**
 * Binary log event definitions. This includes generic code common to all types of log events, as
 * well as specific code for each type of log event. - All numbers, whether they are 16-, 24-, 32-,
 * or 64-bit numbers, are stored in little endian, i.e., the least significant byte first, unless
 * otherwise specified. representation of unsigned integers, called Packed Integer. A Packed Integer
 * has the capacity of storing up to 8-byte integers, while small integers still can use 1, 3, or 4
 * bytes. The value of the first byte determines how to read the number, according to the following
 * table:
 *
 * <table>
...
public abstract class LogEvent {
  ...
  /**
   * NEW_LOAD_EVENT is like LOAD_EVENT except that it has a longer sql_ex, allowing multibyte
   * TERMINATED BY etc; both types share the same class (Load_log_event)
   */
  ...
  /**
   * In some situations, it is necessary to send over ignorable data to the slave: data that a slave
   * can handle in case there is code for handling it, but which can be ignored if it is not
   * recognized.
   */
  ...
  /**
   * 1 byte length, 1 byte format Length is total length in bytes, including 2 byte header Length
   * values 0 and 1 are currently invalid and reserved.
   */
  ...
  /**
   * For an event, 'e', carrying a type code, that a slave, 's', does not recognize, 's' will check
   * 'e' for LOG_EVENT_IGNORABLE_F, and if the flag is set, then 'e' is ignored. Otherwise, 's'
   * acknowledges that it has found an unknown event in the relay log.
   */
