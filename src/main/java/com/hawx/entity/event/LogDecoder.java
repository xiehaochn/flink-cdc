...
import com.hawx.entity.event.innodb.*;
...
public final class LogDecoder {
   ...
   *
   * @return <code>UknownLogEvent</code> if event type is unknown or skipped, <code>null</code> if
   *     buffer is not including a full event.
   */
  ...
  public static LogEvent decode(LogBuffer buffer, LogHeader header, LogContext context)
    ...
    switch (header.getType()) {
      ...
      case LogEvent.PRE_GA_UPDATE_ROWS_EVENT:
        {
          if (logger.isWarnEnabled())
            logger.warn(
                "Skipping unsupported PRE_GA_UPDATE_ROWS_EVENT from: " + context.getLogPosition());
      ...
      case LogEvent.PRE_GA_DELETE_ROWS_EVENT:
        {
          if (logger.isWarnEnabled())
            logger.warn(
                "Skipping unsupported PRE_GA_DELETE_ROWS_EVENT from: " + context.getLogPosition());
      ...
      case LogEvent.BEGIN_LOAD_QUERY_EVENT:
        {
          BeginLoadQueryLogEvent event =
              new BeginLoadQueryLogEvent(header, buffer, descriptionEvent);
          /* updating position in context */
          logPosition.position = header.getLogPos();
          return event;
        }
      case LogEvent.EXECUTE_LOAD_QUERY_EVENT:
        {
          ExecuteLoadQueryLogEvent event =
              new ExecuteLoadQueryLogEvent(header, buffer, descriptionEvent);
          /* updating position in context */
          logPosition.position = header.getLogPos();
          return event;
        }
      ...
      case LogEvent.TRANSACTION_CONTEXT_EVENT:
        {
          TransactionContextLogEvent event =
              new TransactionContextLogEvent(header, buffer, descriptionEvent);
          /* updating position in context */
          logPosition.position = header.getLogPos();
          return event;
        }
