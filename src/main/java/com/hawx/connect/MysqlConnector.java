package com.hawx.connect;

import com.hawx.connect.packet.HeaderPacket;
import com.hawx.connect.packet.MSC;
import com.hawx.connect.packet.MysqlUpdateExecutor;
import com.hawx.connect.packet.PacketManager;
import com.hawx.connect.packet.client.AuthSwitchResponsePacket;
import com.hawx.connect.packet.client.ClientAuthenticationPacket;
import com.hawx.connect.packet.client.QuitCommandPacket;
import com.hawx.connect.packet.server.*;
import com.hawx.connect.socket.BioSocketChannel;
import com.hawx.connect.socket.SocketChannel;
import com.hawx.utils.MySQLPasswordEncrypter;
...
public class MysqlConnector {
  ...
  public void connect() throws IOException {
    if (connected.compareAndSet(false, true)) {
      try {
        ...
        channel = new BioSocketChannel(socket);
  ...
  private void negotiate(SocketChannel channel) throws IOException {
    ...
    if (marker == -2 || marker == 1) {
      ...
      if (pluginName != null && "mysql_native_password".equals(pluginName)) {
        try {
          encryptedPassword =
              MySQLPasswordEncrypter.scramble411(getPassword().getBytes(), authData);
        } catch (NoSuchAlgorithmException e) {
          throw new RuntimeException(
              "can't encrypt password that will be sent to MySQL server.", e);
        }
      } else if (pluginName != null && "caching_sha2_password".equals(pluginName)) {
        isSha2Password = true;
        try {
          encryptedPassword =
              MySQLPasswordEncrypter.scrambleCachingSha2(getPassword().getBytes(), authData);
  ...
  private void auth323(SocketChannel channel, byte packetSequenceNumber, byte[] seed)
      throws IOException {
    // auth 323
  ...
  private byte[] joinAndCreateScrumbleBuff(HandshakeInitializationPacket handshakePacket)
    ...
    System.arraycopy(
        handshakePacket.restOfScrambleBuff,
        0,
        dest,
