...
public class JsonDiffConversion {
  ...
  /**
   * The JSON value in the given path is replaced with a new value. It has the same effect as
   * `JSON_REPLACE(col, path, value)`.
   */
  public static final int DIFF_OPERATION_REPLACE = 0;
  /**
   * Add a new element at the given path. If the path specifies an array element, it has the same
   * effect as `JSON_ARRAY_INSERT(col, path, value)`. If the path specifies an object member, it has
   * the same effect as `JSON_INSERT(col, path, value)`.
   */
  public static final int DIFF_OPERATION_INSERT = 1;
  /**
   * The JSON value at the given path is removed from an array or object. It has the same effect as
   * `JSON_REMOVE(col, path)`.
   */
  public static final int DIFF_OPERATION_REMOVE = 2;

  public static final int JSON_DIFF_OPERATION_COUNT = 3;

  public static StringBuilder print_json_diff(
      LogBuffer buffer, long len, String columnName, int columnIndex, String charsetName) {
    ...
    while (buffer.hasRemaining()) {
      ...
      if (operation_int != DIFF_OPERATION_REMOVE) {
        ...
        JsonConversion.Json_Value jsonValue =
            JsonConversion.parse_value(buffer.getUint8(), buffer, value_length - 1, charsetName);
      ...
      if (!buffer.hasRemaining()
          || operation_names.get(diff_i + 1) != operation_names.get(diff_i)) {
        builder.append(")");
      }
